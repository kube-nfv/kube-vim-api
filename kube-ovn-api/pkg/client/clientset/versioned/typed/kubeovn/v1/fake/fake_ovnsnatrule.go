...
// Apply takes the given apply declarative configuration, applies it and returns the applied ovnSnatRule.
func (c *FakeOvnSnatRules) Apply(ctx context.Context, ovnSnatRule *kubeovnv1.OvnSnatRuleApplyConfiguration, opts metav1.ApplyOptions) (result *v1.OvnSnatRule, err error) {
	if ovnSnatRule == nil {
		return nil, fmt.Errorf("ovnSnatRule provided to Apply must not be nil")
	}
	data, err := json.Marshal(ovnSnatRule)
	if err != nil {
		return nil, err
	}
	name := ovnSnatRule.Name
	if name == nil {
		return nil, fmt.Errorf("ovnSnatRule.Name must be provided to Apply")
	}
	emptyResult := &v1.OvnSnatRule{}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceActionWithOptions(ovnsnatrulesResource, *name, types.ApplyPatchType, data, opts.ToPatchOptions()), emptyResult)
	if obj == nil {
		return emptyResult, err
	}
	return obj.(*v1.OvnSnatRule), err
...
func (c *FakeOvnSnatRules) ApplyStatus(ctx context.Context, ovnSnatRule *kubeovnv1.OvnSnatRuleApplyConfiguration, opts metav1.ApplyOptions) (result *v1.OvnSnatRule, err error) {
	if ovnSnatRule == nil {
		return nil, fmt.Errorf("ovnSnatRule provided to Apply must not be nil")
	}
	data, err := json.Marshal(ovnSnatRule)
	...
	emptyResult := &v1.OvnSnatRule{}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceActionWithOptions(ovnsnatrulesResource, *name, types.ApplyPatchType, data, opts.ToPatchOptions(), "status"), emptyResult)
	if obj == nil {
		return emptyResult, err
	}
	return obj.(*v1.OvnSnatRule), err
