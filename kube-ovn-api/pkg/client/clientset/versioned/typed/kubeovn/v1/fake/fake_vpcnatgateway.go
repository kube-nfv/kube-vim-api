...
// Apply takes the given apply declarative configuration, applies it and returns the applied vpcNatGateway.
func (c *FakeVpcNatGateways) Apply(ctx context.Context, vpcNatGateway *kubeovnv1.VpcNatGatewayApplyConfiguration, opts metav1.ApplyOptions) (result *v1.VpcNatGateway, err error) {
	if vpcNatGateway == nil {
		return nil, fmt.Errorf("vpcNatGateway provided to Apply must not be nil")
	}
	data, err := json.Marshal(vpcNatGateway)
	if err != nil {
		return nil, err
	}
	name := vpcNatGateway.Name
	if name == nil {
		return nil, fmt.Errorf("vpcNatGateway.Name must be provided to Apply")
	}
	emptyResult := &v1.VpcNatGateway{}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceActionWithOptions(vpcnatgatewaysResource, *name, types.ApplyPatchType, data, opts.ToPatchOptions()), emptyResult)
	if obj == nil {
		return emptyResult, err
	}
	return obj.(*v1.VpcNatGateway), err
...
func (c *FakeVpcNatGateways) ApplyStatus(ctx context.Context, vpcNatGateway *kubeovnv1.VpcNatGatewayApplyConfiguration, opts metav1.ApplyOptions) (result *v1.VpcNatGateway, err error) {
	if vpcNatGateway == nil {
		return nil, fmt.Errorf("vpcNatGateway provided to Apply must not be nil")
	}
	data, err := json.Marshal(vpcNatGateway)
	...
	emptyResult := &v1.VpcNatGateway{}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceActionWithOptions(vpcnatgatewaysResource, *name, types.ApplyPatchType, data, opts.ToPatchOptions(), "status"), emptyResult)
	if obj == nil {
		return emptyResult, err
	}
	return obj.(*v1.VpcNatGateway), err
