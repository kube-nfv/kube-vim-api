...
// Apply takes the given apply declarative configuration, applies it and returns the applied providerNetwork.
func (c *FakeProviderNetworks) Apply(ctx context.Context, providerNetwork *kubeovnv1.ProviderNetworkApplyConfiguration, opts metav1.ApplyOptions) (result *v1.ProviderNetwork, err error) {
	if providerNetwork == nil {
		return nil, fmt.Errorf("providerNetwork provided to Apply must not be nil")
	}
	data, err := json.Marshal(providerNetwork)
	if err != nil {
		return nil, err
	}
	name := providerNetwork.Name
	if name == nil {
		return nil, fmt.Errorf("providerNetwork.Name must be provided to Apply")
	}
	emptyResult := &v1.ProviderNetwork{}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceActionWithOptions(providernetworksResource, *name, types.ApplyPatchType, data, opts.ToPatchOptions()), emptyResult)
	if obj == nil {
		return emptyResult, err
	}
	return obj.(*v1.ProviderNetwork), err
...
func (c *FakeProviderNetworks) ApplyStatus(ctx context.Context, providerNetwork *kubeovnv1.ProviderNetworkApplyConfiguration, opts metav1.ApplyOptions) (result *v1.ProviderNetwork, err error) {
	if providerNetwork == nil {
		return nil, fmt.Errorf("providerNetwork provided to Apply must not be nil")
	}
	data, err := json.Marshal(providerNetwork)
	...
	emptyResult := &v1.ProviderNetwork{}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceActionWithOptions(providernetworksResource, *name, types.ApplyPatchType, data, opts.ToPatchOptions(), "status"), emptyResult)
	if obj == nil {
		return emptyResult, err
	}
	return obj.(*v1.ProviderNetwork), err
